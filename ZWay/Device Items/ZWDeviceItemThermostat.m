...
- (void)currentTitle
{
    //decide the current state of the thermostat
...
- (void)updateState
{
    //set the device to the current state
    currentState = [NSString stringWithFormat:@"%@", [self.device.metrics valueForKey:@"currentMode"]];
    [self currentTitle];
...
//hide picker view and send request
...
- (void)sendRequest
{
    ...
    //decide if outdoor or indoor should be used
...
//number of modes in pickerview
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    return 7;
}

//titles for rows in picker
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component
{
    return [states objectAtIndex:row];
}

//decide what mode was selected
